...
async function debugStatsConsistency() {
  console.log('🔍 DEBUG STATS CONSISTENCY'.bold.blue);
  console.log('==========================\n'.bold.blue);
  ...
  try {
    ...
    console.log('📊 VERIFICANDO CONSISTENCIA ENTRE ESTADÍSTICAS Y FILTROS:'.bold);
    console.log('========================================================\n');
    ...
    try {
      ...
      // Calcular total de consultas del resumen
    ...
    // 2. Con filtro de búsqueda "h"
    console.log('2️⃣ CON FILTRO "h":'.cyan);
    try {
      const [resumenResponse, statsResponse] = await Promise.all([
    ...
    // 3. Con filtro de búsqueda "Monica"
    console.log('3️⃣ CON FILTRO "Monica":'.cyan);
    try {
      const [resumenResponse, statsResponse] = await Promise.all([
    ...
    console.error('\n❌ Error general en el debug de consistencia:'.red.bold, error.message);
  }
}

debugStatsConsistency();
