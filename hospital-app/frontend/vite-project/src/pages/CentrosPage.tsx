...
import { AdminApi, type AdminCentro, type AdminCentroCreate } from '../api/adminApi'
import { 
  Activity, 
  Users, 
  LogOut,
...
export default function CentrosPage() {
  ...
  const [centroForm, setCentroForm] = useState<AdminCentroCreate>({
    nombre: '',
    ciudad: '',
    direccion: ''
  })
  ...
  const handleEditCentro = async (e: React.FormEvent) => {
    ...
    try {
      await AdminApi.updateCentro(selectedCentro.id, centroForm)
      setIsEditModalOpen(false)
      setCentroForm({ nombre: '', ciudad: '', direccion: '' })
      setSelectedCentro(null)
      loadData()
    } catch (err) {
      setError("Error al actualizar el centro médico")
      console.error(err)
    }
  }
  ...
  const openEditModal = (centro: AdminCentro) => {
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{new Set(centros.map(c => c.ciudad)).size}</p>
                </div>
              </div>
            </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-500">ID: {centro.id}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
      ...
      {isEditModalOpen && selectedCentro && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full transform transition-all duration-300 scale-100">
            ...
            {/* Contenido del modal */}
            <form onSubmit={handleEditCentro} className="px-8 py-6 space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Nombre del Centro
                </label>
